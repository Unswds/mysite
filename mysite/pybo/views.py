...
def photo_analysis(request):
    ...
    return render(request, 'pybo/photo_analysis.html', {'images': images})
...
def generate_scenes(request):
    ...
    결과 리스트를 같은 main.html에 넘겨줌.
    """
    images = []   # (display_prompt, url) 튜플을 담기 위한 리스트

    if request.method == "POST":
        # 1) POST 요청에서 이미지 파일과 instructions를 가져온다
        uploaded_image = request.FILES.get("image", None)
        instructions = request.POST.get('instructions', '').strip()

        # 2) (선택) 이미지가 업로드된 경우에는 OCR 먼저 수행해서 텍스트를 얻는다.
        if uploaded_image:
            saved_path = default_storage.save(f'temp/{uploaded_image.name}', uploaded_image)
            full_path = os.path.join(settings.MEDIA_ROOT, saved_path)
            ocr_text = extract_text(full_path)
        else:
            ocr_text = ""  # 파일이 없으면 OCR 텍스트는 빈 문자열로 둔다.

        # 3) OCR 텍스트와 사용자가 입력한 추가 요청사항을 합친다.
        #    만약 둘 다 비어있다면, 빈 문자열 전달.
        if ocr_text and instructions:
            combined_text = f"{ocr_text}\n\n추가 요청사항:\n{instructions}"
        elif ocr_text:
            combined_text = ocr_text
        else:
            # 이미지가 없고 OCR 텍스트도 없는 경우: 오직 사용자의 instructions만으로 프롬프트를 생성
            combined_text = instructions

        # 4) GPT에게 프롬프트를 생성해 달라고 요청 (한국어 요약)
        raw_prompts = generate_prompts(combined_text, instructions)

        # 5) instructions가 존재하면 모든 장면에 강제로 삽입
        styled_prompts = []
        for p in raw_prompts:
            # ❶ 공통 요청사항 먼저 → ❷ GPT가 만든 장면 설명 → ❸ 스타일 태그
            if instructions:
                full = f"{instructions}. {p}"
            else:
                full = p
            styled_prompts.append(f"{full}, 웹툰 스타일로 그려주세요.")

        # 6) DALL·E로 이미지 생성 → (styled_prompt, url) 쌍을 받음
        generated = generate_images(styled_prompts)

        # 7) 화면에 보여줄 때는 "원본 프롬프트"와 그에 해당하는 URL을 매칭
        #    generated는 [(styled_prompt, url), ...] 형태이므로, url만 꺼내서 raw_prompts와 짝짓기
        images = [
...
def text_analysis(request):
    """
    GET  : 폼만 렌더링
    POST : 사용자가 입력한 text → GPT 분석 → 결과 렌더링
    """
    if request.method == "POST":
        ...
        if not user_text:
            # 빈 문자열은 다시 폼으로
            messages.error(request, "분석할 텍스트를 입력하세요.")
            return redirect("pybo:text_analysis")
        ...
        # 2) 결과 돌려주기
        context = {"analysis": analysis, "user_text": user_text}
        return render(request, "site1/text_analysis.html", context)
...
def analyze_text(request):
    """
    ① 사용자가 보낸 'message' 를 받아
    ② 이미지 프롬프트 생성 & DALL·E 호출 → 첫 번째 결과 반환
    """
